    ...
    <ul className="mt-2 space-y-1 text-sm text-slate-300">
      {items.map((item) => (
        <li key={item} className="leading-6 flex items-start gap-2">
          <span className="mt-2 h-1 w-1 rounded-full bg-white/50 shrink-0"></span>
          <span>{item}</span>
        </li>
      ))}
    </ul>
          ...
          <div className="mt-3 space-y-2">
            {answer.importantFunctions.map((fn) => (
              <div key={fn.name} className="rounded-lg border border-white/5 bg-black/20 p-3 backdrop-blur-sm">
                <p className="text-sm font-semibold text-white font-mono text-[13px]">{fn.name}</p>
...
function TestsPanel({ answer }) {
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <FlaskConical size={16} className="text-slate-400" aria-hidden="true" />
        ...
        <PanelSection title="Cases">
          <div className="mt-3 space-y-2">
            {answer.testCases.map((testCase) => (
              <div key={testCase.name} className="rounded-lg border border-white/10 bg-white/5 p-3 backdrop-blur-sm">
                <p className="text-sm font-semibold text-white">{testCase.name}</p>
                <p className="mt-1 text-[11px] font-medium uppercase tracking-wider text-slate-400">{testCase.type}</p>
            ...
            {answer.generatedTestFiles.map((file) => (
              <details key={file.path} className="group rounded-lg border border-white/10 bg-[#0c0c0e] text-slate-100 overflow-hidden">
                <summary className="cursor-pointer bg-white/[0.02] px-4 py-2.5 text-sm font-semibold flex items-center hover:bg-white/[0.04] transition-colors">
                  <span className="font-mono text-[12px] text-slate-400">{file.path}</span>
                </summary>
...
function BugsPanel({ answer }) {
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <Bug size={16} className="text-slate-400" aria-hidden="true" />
      ...
      {answer.findings?.length ? (
          ...
          {answer.findings.map((finding) => (
            ...
            >
              <div className="flex items-start gap-2">
                <AlertTriangle size={16} className="text-slate-400 mt-0.5 shrink-0" aria-hidden="true" />
                <div>
              ...
              <div className="mt-3 pl-6 space-y-3">
                ...
                <div className="rounded border border-white/10 bg-white/5 p-3">
                  <p className="text-[11px] font-semibold uppercase text-slate-400 mb-1">Recommendation</p>
                  <p className="text-sm leading-relaxed text-slate-200">{finding.recommendation}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-400 backdrop-blur-sm">
    ...
    <div className="space-y-4">
      {answer.kind === 'architecture_summary' ? (
        <div className="flex items-center gap-2 mb-2">
          <GitFork size={16} className="text-slate-400" aria-hidden="true" />
